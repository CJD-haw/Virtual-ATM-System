...
class ConfirmPinNumber():
      count = 2
      def __init__(self, root, cpin):
            self.operation = None
            self.root = root
            self.offset = ' '
            self.pin = ''
            ...
            self.bg = PhotoImage(file="res/confirm.png")
      ...
      def show(self):      
            self.CONFIRM.config(command=lambda: self.option('confirm from confirm pin'))
            self.HOME.config(command=lambda: self.option('home from confirm pin'))
            self.labc.config(text=' {0} Attemps You Have '.format(ConfirmPinNumber.count+1))
            
            while not self.operation:
                  pass
            
            if self.operation == 'home from confirm pin':
                  ConfirmPinNumber.count = 2
                  return self.operation, ''
            elif self.operation == 'confirm from confirm pin':
                  if self.pin == self.PIN and len(self.pin) == 4 and ConfirmPinNumber.count:
                        return self.operation, self.pin
                  elif ConfirmPinNumber.count:
                        ConfirmPinNumber.count -= 1
                        return 'invalid from confirm pin', ''
                  else:
                        ConfirmPinNumber.count = 2
                        return 'wrong from confirm pin', ''
