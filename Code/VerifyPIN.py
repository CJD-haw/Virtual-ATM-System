...
class Pin():
      ...
      def __init__(self, root, pin):
            ...
            self.bg = PhotoImage(file="res/pin.png")
      ...
      def show(self):      
            self.CONFIRM.config(command=lambda: self.option('confirm from pin'))
            self.EXIT.config(command=lambda: self.option('exit from pin'))
            self.labc.config(text=' {0} Attemps You Have '.format(Pin.count+1))
            while not self.operation:
                  pass
            
            if self.operation == 'exit from pin':
                  Pin.count = 2
                  return self.operation
            elif self.operation == 'confirm from pin':
                  if self.PIN == self.pin and len(self.pin) == 4 and Pin.count:
                        Pin.count = 2
                        return self.operation
                  elif Pin.count:
                        Pin.count -= 1
                        return 'invalid from pin'
                  else:
                        Pin.count = 2
                        return 'wrong from pin'
